...
- Exit-1 immunity requires every NON-transparent chain segment to be diagnostic — a diagnostic later in the chain must not hide a non-diagnostic's failure (`deploy && grep`). Pipe formatters (`select-object`/`tee-object`/...) and navigation (`cd`/`set-location`) are transparent — they never produce the pipeline's exit code, so `tsc | Select-Object` and `cd X && tsc` keep immunity. A real non-diagnostic producer still breaks it (`npm install | select-object` counts)
